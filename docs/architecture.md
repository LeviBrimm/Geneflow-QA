...
6. Background task generates guarded explanations and marks the job completed or failed.
7. Frontend polls `/api/jobs/{job_id}` and then loads `/api/variants/{query_id}`.
8. User can view history and open an HTML report.

## Reliability Surfaces

- Invalid parser input returns `422`.
- Unknown reference variants return `404`.
- Background failures are persisted in `analysis_jobs.error_message`.
...
- Integration tests cover auth, job creation, result retrieval, history, report generation, invalid input, and unknown variants.
